...
PDF:ΑΣΚΗΣΗ ΠΡΑΞΗΣ Ι: ΕΝTOΛΕΣ OΡΙΣΜΟΥ ΔΕΔΟΜΕΝΩΝ (DDL)
Βελτιωμένη-Διορθωμένη έκδοση εργασίας
*/
CREATE DATABASE askisi_praksis_1; 
USE askisi_praksis_1;
###############################################################################################################
# ΑΣΚΗΣΗ 1
###############################################################################################################
...
###############################################################################################################
/*
Λόγο της ολικής συμμετοχής στο Department ο πίνακας Employees θα δώσει το πρωτεύον κλειδί του ως ξένο στον Department.
...
#ΑΣΚΗΣΗ 5
###############################################################################################################
/*Δεν γίνεται η διαγράφη του πίνακα EMPLOYEES γιατί δίνει το κλειδί του στο DEPARTMENTS και στον πίνακα MANAGE.
για να γίνει η διαγραφή θα πρέπει να διαγράψω τον περιορισμό departments_const,manage_const
...
Για να μπορέσω να κάνω διαγραφή τον πίνακα EMPLOYEES θα πρέπει να βγάλω τα ξένα κλειδιά (foreign key):

	ALTER TABLE DEPARTMENTS DROP FOREIGN KEY departments_const;
	ALTER TABLE MANAGE DROP FOREIGN KEY manage_const;
...
*/
#drop database askisi_praksis_1;
